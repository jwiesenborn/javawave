...
// synthesis of a sigh (16-bit)
// composer: kyle vanderburg
// programmer: jesse wiesenborn
// date: 4/28/2019
// version 0.8
...
//Author: Kyle Vanderburg
xhttp.open("GET", "A_Sigh_20180301-d.xml", true);
xhttp.send();
...
function dpmFunction(xml) {
  ...
  for (i = 0; i < parts.length; i++) { // For each part
    ...
    if (parts[i].id == 'P2') {
        ...
        var outputFile = wavChunk(lpcm16voice1ch3.length) + fmtChunk() + dataChunk(lpcm16voice1ch3);
		
		document.getElementById("XMLconsole").innerHTML += "<br/>Sending to download..."; 
        download('sigh3.wav', outputFile); 
